...
use http_client::{CustomHeaders, HttpClient, HttpRequestExt, Method, Request as HttpRequest, RequestBuilderExt, AsyncBody};
use language_model::{
...
impl FetchState {
    ...
    fn refresh_fetch(
        ...
        self.fetch_model_task = Some(cx.spawn(async move |this, cx| {
            let api_key = state.read_with(cx, |state, _cx| {
    ...
    pub fn new(
        ...
        let fetch_state = cx.new(|cx| {
            ...
            cx.observe(&state, move |this: &mut FetchState, _state, cx| {
                ...
                this.refresh_fetch(&observe_id, &observe_http_client, &observe_state, &settings, cx);
            })
            .detach();
